...
-- 数据库引擎: PostgreSQL 14+
-- 字符集: UTF8
-- 说明: 采用BaseEntity统一设计，所有表包含公共审计字段
-- ============================================
...
COMMENT ON COLUMN users.create_by IS '创建者ID';
COMMENT ON COLUMN users.update_by IS '更新者ID';
...
CREATE INDEX idx_users_create_by ON users(create_by);
CREATE INDEX idx_users_update_by ON users(update_by);
...
COMMENT ON COLUMN categories.create_by IS '创建者ID';
COMMENT ON COLUMN categories.update_by IS '更新者ID';
...
CREATE INDEX idx_categories_create_by ON categories(create_by);
CREATE INDEX idx_categories_update_by ON categories(update_by);
...
COMMENT ON COLUMN tags.create_by IS '创建者ID';
COMMENT ON COLUMN tags.update_by IS '更新者ID';
...
CREATE INDEX idx_tags_create_by ON tags(create_by);
CREATE INDEX idx_tags_update_by ON tags(update_by);
...
COMMENT ON COLUMN articles.create_by IS '创建者ID（原user_id）';
...
-- 创建索引
CREATE INDEX idx_articles_create_by ON articles(create_by);
...
CREATE INDEX idx_articles_update_by ON articles(update_by);
...
COMMENT ON COLUMN article_tags.create_by IS '创建者ID';
COMMENT ON COLUMN article_tags.update_by IS '更新者ID';
COMMENT ON COLUMN article_tags.create_time IS '创建时间';
COMMENT ON COLUMN article_tags.update_time IS '更新时间';
COMMENT ON COLUMN article_tags.deleted IS '逻辑删除: 0-未删除, 1-已删除';

CREATE INDEX idx_article_tags_tag_id ON article_tags(tag_id);
CREATE INDEX idx_article_tags_create_by ON article_tags(create_by);
CREATE INDEX idx_article_tags_update_by ON article_tags(update_by);
CREATE INDEX idx_article_tags_deleted ON article_tags(deleted);
...
CREATE TABLE article_likes (
  id BIGSERIAL PRIMARY KEY,
  article_id BIGINT NOT NULL REFERENCES articles(id) ON DELETE CASCADE,
  create_by BIGINT DEFAULT NULL REFERENCES users(id) ON DELETE CASCADE,
  ip_address VARCHAR(45) DEFAULT NULL,
  update_by BIGINT DEFAULT NULL,
  create_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  update_time TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  deleted SMALLINT NOT NULL DEFAULT 0 CHECK (deleted IN (0, 1)),
  UNIQUE(article_id, create_by)
);

COMMENT ON TABLE article_likes IS '文章点赞表';
COMMENT ON COLUMN article_likes.create_by IS '用户ID（原user_id），游客为NULL';
COMMENT ON COLUMN article_likes.ip_address IS 'IP地址';
COMMENT ON COLUMN article_likes.update_by IS '更新者ID';
COMMENT ON COLUMN article_likes.create_time IS '点赞时间';
COMMENT ON COLUMN article_likes.update_time IS '更新时间';
COMMENT ON COLUMN article_likes.deleted IS '逻辑删除: 0-未删除, 1-已删除';

CREATE INDEX idx_article_likes_create_by ON article_likes(create_by);
CREATE INDEX idx_article_likes_update_by ON article_likes(update_by);
CREATE INDEX idx_article_likes_create_time ON article_likes(create_time);
CREATE INDEX idx_article_likes_deleted ON article_likes(deleted);
...
COMMENT ON COLUMN article_favorites.create_by IS '用户ID（原user_id）';
COMMENT ON COLUMN article_favorites.update_by IS '更新者ID';
COMMENT ON COLUMN article_favorites.create_time IS '收藏时间';
COMMENT ON COLUMN article_favorites.update_time IS '更新时间';
COMMENT ON COLUMN article_favorites.deleted IS '逻辑删除: 0-未删除, 1-已删除';

CREATE INDEX idx_article_favorites_create_by ON article_favorites(create_by);
CREATE INDEX idx_article_favorites_update_by ON article_favorites(update_by);
CREATE INDEX idx_article_favorites_create_time ON article_favorites(create_time);
CREATE INDEX idx_article_favorites_deleted ON article_favorites(deleted);
...
CREATE INDEX idx_comments_create_by ON comments(create_by);
CREATE INDEX idx_comments_update_by ON comments(update_by);
...
COMMENT ON COLUMN navigation_categories.create_by IS '创建者ID';
COMMENT ON COLUMN navigation_categories.update_by IS '更新者ID';
...
CREATE INDEX idx_navigation_categories_create_by ON navigation_categories(create_by);
CREATE INDEX idx_navigation_categories_update_by ON navigation_categories(update_by);
...
COMMENT ON COLUMN navigation_sites.create_by IS '创建者ID';
COMMENT ON COLUMN navigation_sites.update_by IS '更新者ID';
...
CREATE INDEX idx_navigation_sites_create_by ON navigation_sites(create_by);
CREATE INDEX idx_navigation_sites_update_by ON navigation_sites(update_by);
...
('前端开发', 'frontend', 'Web前端技术分享', 100, 1, 1),
('后端开发', 'backend', '服务端技术探讨', 90, 1, 1),
('数据库', 'database', '数据库设计与优化', 80, 1, 1),
('DevOps', 'devops', '运维与部署相关', 70, 1, 1),
('算法', 'algorithm', '数据结构与算法', 60, 1, 1),
('随笔', 'essay', '生活随笔', 50, 1, 1);
...
(1, 1, 1, 1), (1, 3, 1, 1),  -- Vue 3 文章关联 Vue 和 TypeScript
(2, 9, 1, 1), (2, 4, 1, 1),  -- Spring Boot 文章关联 Spring Boot 和 Node.js
(3, 5, 1, 1),                  -- PostgreSQL 文章关联 PostgreSQL
(4, 7, 1, 1),                  -- Docker 文章关联 Docker
(5, 8, 1, 1);                  -- 算法文章关联算法
...
-- 插入导航站点
INSERT INTO navigation_sites (category_id, name, description, url, icon, sort_order, create_by, update_by) VALUES
(1, 'VS Code', '微软开发的代码编辑器', 'https://code.visualstudio.com/', '/icons/vscode.png', 100, 1, 1),
(1, 'JetBrains', '专业的开发工具套件', 'https://www.jetbrains.com/', '/icons/jetbrains.png', 90, 1, 1),
(2, 'MDN', 'Web 开发权威文档', 'https://developer.mozilla.org/', '/icons/mdn.png', 100, 1, 1),
